...
如果使用 `fake-ip`，为保证手机 WiFi 先稳定有网，可以让 `start_clash.sh` 在 TUN 开启时加入两条高优先级策略：
...
```text
DNS 使用 redir-host / 真实 IP
LAN_TUN_BYPASS=0
LAN 手机流量进入 TUN，由规则分流：
  cn / private -> DIRECT
  Google / 国外 -> PROXY[REALITY]
```
...
如果后续想让 LAN 手机完全绕过 TUN，只保留路由器本机 TUN，可临时开启保护：

```sh
LAN_TUN_BYPASS=1 /data/clash/start_clash.sh
```
